  ...
  return (
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
      {services.map((service, index) => (
        ...
        >
          {/* Static overlay for title and hint */}
          <div className="absolute bottom-0 left-0 right-0 p-4 bg-blue-950 bg-opacity-50 text-white z-10 flex items-center justify-between">
            <h3 className="text-lg font-semibold">{service.title}</h3>
          </div>

          {/* Hover overlay */}
          <div className="absolute inset-0 bg-black bg-opacity-70 flex flex-col justify-end p-6 text-white opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-20">
            <h3 className="text-lg font-semibold">{service.title}</h3>
